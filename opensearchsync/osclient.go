...
// EnsureIndex creates the target index with the adstash mappings if it does not exist. It is a
// no-op when the index (or its write alias) is already present. Additive mapping patches on an
// existing index are left to the operator (or a future enhancement).
func (c *osBulkClient) EnsureIndex(ctx context.Context, index string) error {
	...
	if err == nil && exists.StatusCode == http.StatusOK {
		return nil // present (index or alias)
	}
	...
	if err != nil {
		// A concurrent creator (or an existing alias) can race us; treat "already exists" as ok.
